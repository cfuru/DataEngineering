...
1. **UDF Definition Notebook:** Create `Databricks/Notebooks/utils/udf_helpers.ipynb` that defines Python functions and registers them as Spark SQL UDFs using `spark.udf.register(udf_name, python_func, return_type)`.
...
4. **Notebook Integration:** Update `Databricks/Notebooks/silver/soldObjects.ipynb` to:
...
5. **Unit Test Coverage:** Create or update `tests/test_pipeline_helpers.py` to include at least one test that:
   ...
   - Runs successfully with `pytest tests/test_pipeline_helpers.py`.
...
2. **Rewriting Other Notebooks:** Only refactor `Databricks/Notebooks/silver/soldObjects.ipynb` in this work. Other notebooks (if they exist) can be updated in future work.
...
3. **Testing Strategy:** Write unit tests in `tests/test_pipeline_helpers.py` that directly import and test the Python functions (not via Spark SQL). This allows fast, local testing without a Databricks cluster.
...
1. **Unit Test Coverage:** At least one unit test in `tests/test_pipeline_helpers.py` covers the transformation function (`normalize_source_filename`) and passes locally with `pytest`.

2. **Notebook Execution:** The refactored `Databricks/Notebooks/silver/soldObjects.ipynb` runs end-to-end in Databricks without errors, producing the same results as before the refactoring.

3. **Code Reusability:** The `udf_helpers.ipynb` notebook can be run (`%run ../utils/udf_helpers`) from any notebook in the `Databricks/Notebooks/silver/` directory, and UDFs are immediately available in `spark.sql()` statements and DataFrame expressions.
